...
N_feat = N_ex*19;        % Number of features
...
for ii = 1:N
    ...
    for jj = 1:19 % Use 19 sensors data
        ...
        FeatMat_train(ii,N_ex*(jj-1)+1) = mean(data_i1);       % feature 1: Mean
        FeatMat_train(ii,N_ex*(jj-1)+2) = std(data_i1);        % feature 2: STD
        FeatMat_train(ii,N_ex*(jj-1)+3) = min(data_i1);        % feature 3: Minimum
        FeatMat_train(ii,N_ex*(jj-1)+4) = max(data_i1);        % feature 4: Maximum 
        ...
        FeatMat_train(ii,N_ex*(jj-1)+5) = mean(data_i2);       % feature 1: Mean
        FeatMat_train(ii,N_ex*(jj-1)+6) = std(data_i2);        % feature 2: STD
        FeatMat_train(ii,N_ex*(jj-1)+7) = min(data_i2);        % feature 3: Minimum
        FeatMat_train(ii,N_ex*(jj-1)+8) = max(data_i2);        % feature 4: Maximum 
...
for ii = 1:N
    ...
    for jj = 1:19 % Use #3 to #21 from data
        ...
        FeatMat_test(ii,N_ex*(jj-1)+1) = mean(data_i1);       % feature 1: Mean
        FeatMat_test(ii,N_ex*(jj-1)+2) =  std(data_i1);       % feature 2: STD
        FeatMat_test(ii,N_ex*(jj-1)+3) =  min(data_i1);       % feature 3: Minimum
        FeatMat_test(ii,N_ex*(jj-1)+4) =  max(data_i1);       % feature 4: Maximum 
        % FeatMat_test(ii,N_ex*(jj-1)+5) = peak2peak(data_i1);   % feature 5: Peak-to-peak
        %FeatMat_test(ii,N_ex*(jj-1)+5) = kurtosis(data_i1);  % feature 5: Kurtosis

        FeatMat_test(ii,N_ex*(jj-1)+5) = mean(data_i2);       % feature 1: Mean
        FeatMat_test(ii,N_ex*(jj-1)+6) =  std(data_i2);       % feature 2: STD
        FeatMat_test(ii,N_ex*(jj-1)+7) =  min(data_i2);       % feature 3: Minimum
        FeatMat_test(ii,N_ex*(jj-1)+8) =  max(data_i2);       % feature 4: Maximum 

%         FeatMat_test(ii,N_ex*(jj-1)+6) = mean(data_i2);       % feature 1: Mean
%         FeatMat_test(ii,N_ex*(jj-1)+7) = std(data_i2);        % feature 2: STD
%         FeatMat_test(ii,N_ex*(jj-1)+8) = min(data_i2);        % feature 3: Minimum
%         FeatMat_test(ii,N_ex*(jj-1)+9) = max(data_i2);        % feature 4: Maximum 
        % FeatMat_test(ii,N_ex*(jj-1)+10)= peak2peak(data_i2);   % feature 5: Peak-to-peak
        %FeatMat_test(ii,N_ex*(jj-1)+10)= kurtosis(data_i2);   % feature 5: Kurtosis
...
% trick
% FeatMat_train(1,:) = FeatMat_train(1,:) + 0.0001;
