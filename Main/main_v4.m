...
%
dir_truth = 'C:\Users\minamitu\OneDrive - University of Cincinnati\_CLASS\2-1_IndustrialAI\Final\_Shared\truth';
cd(dir_truth)
...
%% plot confusion matrix for SOM-MQE
figure;

truth = categorical( truth ); 

predict = categorical( predict );

plotconfusion(truth,predict)

title('SOM-MQE - Confusion Matrix')


%% plot confusion matrix for PCA-SPE
figure;

truth = categorical( truth ); 

predict = categorical( SPE_predict );

plotconfusion(truth,predict)

title('PCA-SPE - Confusion Matrix')

%% plot confusion matrix for T2
figure;

truth = categorical( truth ); 

predict = categorical( T2_predict );

plotconfusion(truth,predict)

title('PCA-T2 - Confusion Matrix')
